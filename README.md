...
Despite academic progress, real-world clinical adoption remains uneven. While adoption is growing in some domains, such as imaging, it is still limited in many areas, specifically primary healthcare and diagnosis, despite substantial progress in AI methods for early detection. This is not surprising, clinical diagnosis directly affects patient outcomes, and doctors are understandably cautious about relying on tools they do not fully understand. This is why there is a need for lightweight and interpretable AI models that are not only deployable in resource-constrained settings but also easily interpretable by medical professionals. 

The aim of this project is to develop an AI-based cardiac abnormality detector using Decision Trees, which are inherently lightweight and interpretable. This project will focus on two things: first, lightweightness and computational simplicity, the model should be able to give satisfactory results with low computational requirements, particularly for short-duration ECG recordings such as portable or point-of-care tests that are no longer than a few minutes. Second, interpretability, the model's decision-making should be easily understood by the primary users, that is, the primary physicians who are first to identify any problem with a patient and refer them to a specialist.
